...
class sources:
                ...
                if select == '1' and 'plugin' in control.infoLabel('Container.PluginName'):
                    ...
                    if self.content == 'episode' and control.setting('enable.upnext') == 'true':
                        ...
                        try:
                            ...
                            AddonSignals.registerSlot('upnextprovider', return_id, self.sourcesDirect(items))
                        except:
                            import traceback
                            traceback.print_exc()
                            pass
    ...
    def sourcesFilter(self):
        ...
        if sortthemup == '': sortthemup = 'false'
        quality = control.setting('hosts.quality')
        if quality == '': quality = '0'
        ...
        for d in debrid.debrid_resolvers:
            ...
            if sortthemup == 'true':
                filter += [dict(i.items() + [('debrid', d.name)]) for i in self.sources if 'magnet:' in i['url']] #### Try it this way. Seems to be working good.
                filter += [dict(i.items() + [('debrid', d.name)]) for i in self.sources if i['source'] in valid_hoster and 'magnet:' not in i['url']] ### This Too
            else:
                filter += [dict(i.items() + [('debrid', d.name)]) for i in self.sources if i['source'] in valid_hoster or 'magnet:' in i['url']] ### This Too
        ...
        if prem_identify == '': prem_identify = 'magenta'
    ...
    def sourcesResolve(self, item, info=False):
        try:
            ...
            if url == None or (not '://' in url and not local and 'magnet:' not in url): raise Exception() ## Changes here
            if not local:
    ...
    def getLanguage(self):
        langDict = {'English': ['en'], 'German': ['de'], 'German+English': ['de','en'], 'French': ['fr'], 'French+English': ['fr', 'en'], 'Portuguese': ['pt'], 'Portuguese+English': ['pt', 'en'], 'Polish': ['pl'], 'Polish+English': ['pl', 'en'], 'Korean': ['ko'], 'Korean+English': ['ko', 'en'], 'Russian': ['ru'], 'Russian+English': ['ru', 'en'], 'Spanish': ['es'], 'Spanish+English': ['es', 'en'], 'Italian': ['it'], 'Italian+English': ['it', 'en'], 'Greek': ['gr'], 'Greek+English': ['gr', 'en']}
    ...
    def getConstants(self):
        ...
        self.hostcapDict = ['hugefiles.net', 'kingfiles.net', 'openload.io', 'openload.co', 'oload.tv', 'thevideo.me', 'vidup.me', 'streamin.to', 'torba.se', 'flashx.tv', 'vshare.eu', 'vshare.io']
        self.hosthqDict = ['gvideo', 'google.com', 'openload.io', 'openload.co', 'oload.tv', 'thevideo.me', 'rapidvideo.com', 'raptu.com', 'filez.tv', 'uptobox.com', 'uptobox.com', 'uptostream.com', 'xvidstage.com', 'streamango.com', 'vev.io']
        self.hostblockDict = []
    ...
    def next_info(self):
        ...
        current_episode["tvshowid"] = current_info['imdb'], current_info['tvdb']
        ...
        next_episode["tvshowid"] = next_info['imdb'], next_info['tvdb']
