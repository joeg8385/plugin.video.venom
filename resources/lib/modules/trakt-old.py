...
def _error(url, post, timestamp, message):
    ...
    if control.setting('trakt.notifications'):
        control.infoDialog(control.lang(32315).encode('utf-8'), heading=str(name), sound=True, icon=icon)
    control.idle()
    sys.exit()
    return None
...
def watch(imdb = None, tvdb = None, season = None, episode = None, refresh = True, notification = False):
    ...
    if notification: control.infoDialog(control.lang(32315).encode('utf-8'), heading=str(name), sound=True, icon=icon)
...
def manager(name, imdb, tvdb, content, refresh = True):
    try:
        ...
        # if select:
            ...
            result = getTrakt('/users/me/lists', post = {"name": new, "privacy": "private"})[0]
            ...
            result = getTrakt(items[select][1] % slug, post = post)[0]
        else:
            result = getTrakt(items[select][1], post = post)[0]
